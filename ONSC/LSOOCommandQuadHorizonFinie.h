...
class LSOOCommandQuadHorizonFinie : public LSOO<T>
{
    ...
    LSOOCommandQuadHorizonFinie(int nbrCommand, Mat<T> (*F)(Mat<T>,Mat<T>,double), Mat<T> dX, double dt, double horizonT, T bound, bool BOZ = false )
    {
        ...
        Q = Mat<T>((T)0,5,5);
        for(int i=Q.getLine();i--;)   Q.set((T)1e40,i+1,i+1);
        //Q.set((T)1e4,3,3);
        //Q.set((T)0,4,4);
        //Q.set((T)0,5,5);
        
        R = Mat<T>((T)0,2,2);
        for(int i=R.getLine();i--;)   R.set((T)1e1,i+1,i+1);
    ...
    {   
		...
		*/
		/*----------------------------*/
	        ...
	        costState += transpose( state[i]-dX)*(Q * (state[i]-dX) );
	}
        
        Mat<T> costCommand(transpose(u[0])*(R*u[0]) );
        for(int i=1;i<nbrCommand*nbrStatePerBOZ;i++)   costCommand += transpose( u[i])*(R*u[i]);
        ...
        for(int j=0;j<nbrState;j+=nbrStatePerBOZ)
        {
		...
		*/
		/*----------------------------*/
        	...
        	tempxj = transpose(state[j])*( ((T)2)*Q );
        	Mat<T> tempA((T)0,state[0].getLine(),state[0].getLine());
	        for(int i=tempA.getLine();i--;)	tempA.set((T)1,i+1,i+1);	        	       	     
	        
	        Mat<T> tempSum((T)0,tempA.getLine(), /*watchout only that : */nbrCommand*R.getLine() );
        	for(int jj=0;jj<=j;jj+=nbrStatePerBOZ)
        	{
        		Mat<T> tempDeriv((T)0, R.getLine(), tempSum.getColumn());
        		for(int i=R.getLine();i--;)	tempDeriv.set( (T)1, i+1, jj*R.getLine()+i+1);
        		
        		for(int k=j-jj;k--;)	tempA *= A[k];
        		
        		tempSum += tempA*(B[jj]*tempDeriv);
        	}
        	 
        	costState += tempxj*tempSum;
        }
        ...
        for(int j=0;j<nbrState;j+=nbrStatePerBOZ)
        {
        	tempuj = transpose(u[j])*( ((T)2)*R );
        	
	        Mat<T> tempDeriv((T)0, 1, /*watchout : */nbrCommand*R.getLine());
       		for(int i=tempuj.getColumn();i--;)	tempDeriv.set( tempuj.get(1,i+1) , 1, j*R.getLine()+i+1);
        	 
        	costCommand += tempDeriv;
        }
    ...
    Mat<T> init()
    {
        ...
        NormalRand gen(0, this->bound/2,(long)100);
